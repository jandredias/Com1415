// $Id: xml_writer.h,v 1.7 2015/05/19 15:10:49 ist175741 Exp $ -*- c++ -*-
...
namespace pwn {
  ...
  private:
    inline void openCloseInlineTag(const std::string &tag, const std::string &value, int lvl){
		os() << std::string(lvl, ' ') + "<" + tag + ">" << value << "<" + tag + "/>" << std::endl;
    }
    inline void openCloseTag(const std::string &tag, int lvl){
